...
\Procedure{Update}{}
\If{$\textit{AnyBallInTrigger}$} 
\If{$\textit{SpaceKeyIsPressed}$} 
\If{$\textit{power} <= \textit{maxPower}$} 
\State $\textit{power} \gets \textit{power} + \textit{stepPower} * \textit{deltaTime}$
\EndIf 
\EndIf 
\If{$\textit{SpaceKeyIsReleased}$} 
\State $\textit{ballForce} \gets \textit{power}$
\EndIf
\EndIf
\EndProcedure
\Procedure{OnTriggerExit}{}
